...
class Everything:
    ...
    def updatewindow(self):
        if(self.Health<0):
            window.create_image(200, 200, image=Win)
        
        window.create_image(self.playerposX, self.playerposY, image=trump)
        if(self.Health>0):
            window.create_image(150, 150, image=Jean)
        else:
            window.create_text(300,150,text="YOU WIN",font=("Purisa", 100))
        #bullet logicality

        for singleBullet in self.Bullets:
#
            if(singleBullet.xpos >100 and singleBullet.xpos <200 and singleBullet.ypos >100 and singleBullet.ypos <200):
                self.Health -=1
